...
class Driver:
    ...
    def doServe(self, args):
        ...
        for id in args:
            ...
            if disabled:
                self.servers[id] = s
                self.console.outMsg("Disabled server %s set up at %s:%d" %
                                    (id, self.host, port))
            elif s.running:
                self.servers[id] = s
                self.console.outMsg("Server %s running at %s:%d" %
                                    (id, self.host, port))
            else:
    ...
    def doRequestOrFetch(self, args, isFetch):
        ...
        if not self.haveProxy:
            command = "Fetch" if isFetch else "Request"
            self.console.errMsg("%s requires there to be a running proxy" %
                                command)
            return False
        rid = args[0]
        file = args[1]
        ...
        try:
            event = self.eventManager.addRequestEvent(rid,
                                                      server=sid,
                                                      isFetch=isFetch)
        except events.EventException as ex:
            self.console.errMsg("Couldn't generate request event %s (%s)" %
                                (rid, ex))
            return False
    ...
    def doRespond(self, args):
        ...
        for rid in args:
            ...
            if event is None:
                ...
                self.console.outMsg(
                    "Server responded to request %s with status %s%s" %
                    (rid, event.tag, reason))
        return ok
    ...
    def doDelay(self, args):
        ...
        ok = len(pending) == 0
        if not ok:
            self.console.errMsg("Warning.  %d events still pending" %
                                len(pending))
            for e in pending:
    ...
    def doWait(self, args):
        ...
        if not ok:
            self.console.errMsg(
                "Warning.  %d events still pending after timeout of %d milliseconds"
                % (len(pending), ms))
            for e in pending:
                self.console.outMsg("  " + str(e))
        return ok
    ...
    def doCheck(self, args):
        ...
        if checkTag != event.tag:
            ...
            self.console.errMsg(
                "Request %s generated status '%s'.  Expecting '%s'%s" %
                (rid, event.tag, checkTag, sreason))
            if self.autoTrace.getBoolean():
                self.trace(rid)
            return False
        self.console.outMsg("Request %s yielded expected status '%s'" %
                            (rid, event.tag))
        return True
    ...
    def doProxy(self, args):
        ...
        try:
            ...
            self.console.errMsg("Couldn't start proxy %s (%s)" %
                                (path, str(e)))
            return False
        mout = files.FileMonitor(self.proxyProcess.stdout,
                                 self.console,
                                 id="Proxy stdout")
        merr = files.FileMonitor(self.proxyProcess.stderr,
                                 self.console,
                                 id="Proxy stderr")
        ...
        if self.proxyProcess.poll():
            self.console.errMsg(
                "Proxy process terminated with return code %d" %
                self.proxyProcess.returncode)
            self.proxyProcess = None
            return False
        self.requestManager.proxy = (self.host, port)
        self.haveProxy = True
        self.console.outMsg("Proxy set up at %s:%d" %
                            self.requestManager.proxy)
        return True
    ...
    def doExternalProxy(self, args):
        ...
        if len(args) == 1:
            ...
            if len(fields) != 2:
                self.console.errMsg(
                    "Require proxy to be specified as 'host:port'")
                return False
            host = fields[0]
            sport = fields[1]
        elif len(args) == 2:
