...
static char *free_listp = NULL; /* Pointer to free block */
...
{
    // 初始化堆哨兵 start
    if ((heap_listp = mem_sbrk(2 * WSIZE + MINSIZE)) == (void *)-1)
    {
        return -1;
    }

    PUT(heap_listp, 0);                              /* 双字对齐填充 */
    PUT(heap_listp + (1 * WSIZE), PACK(MINSIZE, 1)); /* 序言头 */
    PUT_PREV(heap_listp + (2 * WSIZE), 0);           /* free root prev */
    PUT_NEXT(heap_listp + (2 * WSIZE), 0);           /* free root next */
    PUT(heap_listp + (6 * WSIZE), PACK(MINSIZE, 1)); /* 序言尾 */
    PUT(heap_listp + (7 * WSIZE), PACK(0, 1));       /* 结尾块 */
    // 初始化堆哨兵 end
...
static void *find_fit(size_t asize)
{
    ...
    for (fp = GET_NEXT(free_listp); fp != 0; fp = GET_NEXT(fp))
    {
...
{
    unsigned long next = GET_NEXT(free_listp);
    PUT_NEXT(free_listp, fp);

    PUT_NEXT(fp, next);
    PUT_PREV(fp, free_listp);
...
static void checkheap(int verbose)
{
    ...
    /* 头尾块 */
    if ((GET_SIZE(HDRP(heap_listp)) != MINSIZE) || !GET_ALLOC(HDRP(heap_listp)))
    {
